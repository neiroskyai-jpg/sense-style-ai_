...
# Стилевые маркеры В НАЗВАНИИ вещи. Зачем: раньше стиль вещи наследовался от БРЕНДА, и все 154
# вещи Lichi имели «drama; romance» — юбка с воланами и строгий костюмный жакет были для системы
# одинаковы. Классической клиентке выдавались кардиган в горошек и спортивная куртка, а 14
# классических жакетов того же бренда до неё не доходили.
#
# Берём ДЕТАЛИ И КРОЙ, а не ткани и категории: шерсть/хлопок бывают в любом стиле (шерстяной
# жакет — классика, шерстяной оверсайз — натуральный), а «жакет» — это слот, не характер.
_STYLE_MARKERS = {
    "romance": ["волан", "кружев", "драпир", "бант", "рюш", "цветочн", "горошек", "баск",
                "пышн", "вышивк", "кроше", "плиссе", "сборк", "перламутр", "воротник-стойк",
                "romantic", "с завязкой на шее"],
    "classic": ["прямо", "приталенн", "однобортн", "двубортн", "костюмн", "стрелк", "футляр",
                "классическ", "лодочк", "строг", "тренч", "лаконичн", "сдержанн"],
    "drama": ["асимметр", "косух", "контрастн", "металлик", "экстравагант", "байкер", "сетк",
              "лаковы", "объемн", "объёмн", "на одно плечо", "разрез", "корсет", "шипы"],
    "natural": ["лен ", "льн", "деним", "джинс", "оверсайз", "свободн", "худи", "фланел",
                "вязан", "кроссовк", "утилитарн", "карго", "кэжуал", "casual"],
}


def _style_from_name(*texts: str) -> set[str]:
    """Стилевые поля вещи по её названию (детали кроя). Пусто — если маркеров нет."""
    hay = " ".join(t or "" for t in texts).lower()
    return {field for field, kws in _STYLE_MARKERS.items() if any(k in hay for k in kws)}

# Категории-носители базового стиля. Раньше был заполнен ТОЛЬКО natural — и классическая клиентка
# не получала бонуса за жакет: подходящий приталенный жакет в её палитре набирал столько же, сколько
# юбка с воланами, и проигрывал ей просто по порядку в каталоге. Отсюда «где мои жакеты».
_FORMULA_CATEGORIES = {
    "natural": ["брюки", "трикотаж", "джемпер", "кардиган", "рубашка", "джинс", "деним",
                "юбка", "платье", "лоферы", "ботинки", "челси", "тренч", "шоппер"],
    "classic": ["жакет", "пиджак", "блейзер", "брюки", "юбка", "рубашка", "блуза", "платье",
                "тренч", "пальто", "лодочки", "лоферы", "водолазка", "сумка"],
    "romance": ["платье", "юбка", "блуза", "топ", "кардиган", "лодочки", "балетки", "сарафан",
                "туфли", "клатч", "палантин"],
...
def score_products(profile: dict, products: list[Product]) -> list[tuple[float, Product]]:
    ...
    for p in products:
        ...
        own_styles = _style_from_name(p.name)
        prod_styles = own_styles or _style_set(p.style_fields)
        if client_styles and prod_styles:
            if client_styles & prod_styles:                    # стиль вещи совпал с доминантой — плюс
                score += 2.0 if own_styles else 1.5            # своя разметка надёжнее брендовой
