...
ЭТО ВИТРИНА ПРОДУКТА. Клиентка видит эти два образа сразу после диагностики и по ним решает, \
идти ли дальше. Образ должен читаться как съёмка из модного журнала, а не как «что-то удобное \
из масс-маркета». Каждая вещь названа конкретно: крой, длина, ткань, цвет из её палитры.

АКТУАЛЬНОСТЬ (сезон 2026-2027, обязательно):
— Силуэт: приталенность и структура вернулись. Узкий жакет, зауженные или широкие брюки с \
чёткой линией, акцент на талии, мягко-квадратное плечо. Оверсайз — только осознанным объёмом \
сверху при узком низе, никогда как «мешок».
— Длины: чем шире брюки, тем длиннее (в пол); чем уже — тем короче (7/8 до щиколотки). \
Юбки и платья — явное миди/мидакси или явное мини, без промежуточных «спорных» длин.
— Ткани и фактуры сезона: шерсть, кашемир, шёлк, плотный хлопок, кожа (вишня, бордо), бархат, \
крупный воротник, кружево точечно. Цвета: hot chocolate, вишня-бордо, глубокий синий, умбра, \
горчица, пудра — поверх нейтральной базы.
— ЗАПРЕЩЕНО (читается как немодное): бесформенный вязаный кардиган-оверсайз, рукав 3/4 и \
подвёрнутые рукава, скинни, выбеленный или желтоватый деним, рванки и стразы, дешёвая меховая \
опушка, шуба-тедди, длинные угги и дутики, микро-сумки и рюкзаки, поясная сумка-бананка, \
мягкое рыхлое «ленивое» пальто, смешение тёплого и холодного бежа в одном образе.

КУДА НАДЕТЬ (wear_to): 2-3 КОНКРЕТНЫХ повода из жизни клиентки — «переговоры с новым
клиентом», «ужин в ресторане», «конференция», «встреча с друзьями», «поездка в другой город».
Не абстракции вроде «на каждый день» и «для особых случаев»: они ничего не говорят.
Поводы двух направлений НЕ должны совпадать — иначе непонятно, зачем их два.

Клиентка сначала читает, ЧТО это за образ и КУДА его надеть, и только потом — из чего он собран.
Список вещей вторичен: он отвечает на «как повторить», а не на «зачем мне это».

КАПСУЛЬНОСТЬ: два образа — это начало её капсулы, а не два случайных комплекта. Вещи должны \
сочетаться МЕЖДУ образами: низ из первого обязан работать с верхом из второго. Держи общую \
палитру (2-3 нейтральные базы + 1 акцент), чтобы из этих вещей собирались и другие комплекты.
...
Название описывает, КАК ПРОЧИТАНА её Формула, а не новый стиль: «Мягкая версия», «Собранная \
версия», «Тихое прочтение», «Сильное прочтение», «Спокойный вариант», «Уверенный вариант». \
...
СЛОВО «РЕГИСТР» КЛИЕНТКЕ НЕ ПОКАЗЫВАЕМ. Это наш внутренний термин: «Спокойный регистр» она \
читает как техническую пометку, а не как название своего образа. Пиши по-человечески.
...
и подобное. НЕ выдуманное название стиля>",
      ...
      "wear_to": ["<повод1>", "<повод2>", "<повод3>"],
      "items": ["<вещь1>", "<вещь2>", "<вещь3>", "<вещь4>", "<вещь5>"],
      "image_generation_prompt": "<английский промпт ОДЕЖДЫ и СЦЕНЫ образа. \
ПЕРЕЧИСЛИ РОВНО ТЕ ЖЕ ВЕЩИ, что в items выше, в том же составе и тех же цветах — \
это описание ОДНОГО образа двумя языками, а не два разных образа. \
База образа (dress / skirt+jacket / trousers+blazer) должна ОТЛИЧАТЬСЯ от базы второго \
направления. Палитра, силуэт под фигуру, фотостиль и свет под характер направления. \
БЕЗ описания лица/личности — берётся с фото. ~60 слов>"
    },
    { … второе направление … }
  ]
}"""
...
_REGISTER_OK = ("верси", "прочтен", "вариант", "мягк", "собран", "тих", "сильн",
                "спокойн", "уверенн")

# Наш внутренний жаргон, который не должен доезжать до клиентки. «Спокойный регистр» читается
# как техническая пометка, а не как название её образа: слово из методички, а не из жизни.
_JARGON = ("регистр", "семантическ", "дескриптор", "паттерн прочтения")
...
def _canonical_direction_names(directions: list[dict], diagnosis: dict) -> list[dict]:
    ...
    for i, d in enumerate(directions):
        ...
        elif any(w in low for w in _JARGON):
            # Смысл названия верный, испорчено только слово: «Спокойный регистр» → «Спокойный
            # вариант». Менять на безликую «Мягкую версию» жалко — потеряется характер направления.
            d["name"] = _strip_jargon(name)
        out.append(d)
    return out


def _strip_jargon(name: str) -> str:
    """Убрать наш термин из названия, сохранив его смысл.

    «Регистр» меняется на «вариант» — характер направления («Спокойный», «Уверенный») остаётся.
    Остальной жаргон осмысленной замены не имеет, поэтому такое название просто отбрасываем.
    """
    fixed = re.sub(r"\bрегистр\w*", "вариант", name, flags=re.IGNORECASE)
    fixed = re.sub(r"\s+", " ", fixed).strip(" -—·")
    if not fixed or any(w in fixed.lower() for w in _JARGON):
        return "Ещё одно прочтение"
    return fixed
...
_CAPSULE_QUALITY_RULES = (
    ...
    "11. ЦВЕТОВАЯ ПАРА В ОСНОВЕ ОБРАЗА: образ строится на ПАРЕ цветов из палитры, а не на случайном "
    "наборе. Правило трио: не больше трёх цветов в образе — два нейтрала + один акцент. Принт "
    "(леопард, полоска, клетка) считается за нейтраль, а не за акцент.\n"
    "12. АКЦЕНТ ЗВУЧИТ ДВАЖДЫ: акцентный цвет образа повторяется минимум в двух точках "
    "(сумка + обувь, верх + украшение, пояс + шарф). Одинокое цветовое пятно читается как "
    "случайность, а не как решение стилиста.\n"
    "13. ОБУВЬ И СУМКА — в тон ДРУГ ДРУГУ, а не в тон одежде. Именно они держат образ; "
    "разнобой обуви и сумки рассыпает даже верно собранный комплект.\n"
    "14. РЕВЕРС РОЛЕЙ: если два образа строятся на одной цветовой паре, они обязаны различаться тем, "
    "КАКОЙ цвет несёт объём, а какой — акцент (голубой верх + шоколадный низ ↔ шоколадный верх + "
    "голубой низ). Две вариации одной пары в одинаковых ролях — потерянный образ."
)
...
_STYLING_SYSTEM = (
    ...
    "Образы обязаны различаться ДВАЖДЫ: по сценарию И по цветовым ролям — в первом образе акцентный "
    "цвет держит верх, во втором уходит вниз или в аксессуары (реверс ролей). Акцент в каждом образе "
    "звучит минимум в двух точках; обувь и сумка — в тон друг другу, а не одежде.\n"
...
_TREND_CANON = (
    ...
    # Доза и граница. Клиентка 25-45 одевается для работы и статуса, а не для подиума.
