...
ПРОПОРЦИЯ 60/30/10. Формула комбинаторная, и без заданных долей образ выходит размазанным — \
ни одно поле не ведёт, а формула читается как склейка. Доминирующее поле (наибольший процент \
в semantic_field_distribution) держит 60%: силуэт и базовая палитра. Вторичное — 30%: фактура \
и одна-две вещи-детали. Акцентное — 10%: РОВНО ОДИН выразительный элемент (украшение, обувь \
или цвет), не два. Для формулы из двух полей: 70 доминанта / 30 вторичное.
...
_STYLING_SYSTEM = (
    ...
    # Разбор фаундера по реальной паре: приём был заявлен, но не показан. Якорем назвали брюки, а
    # держал оба образа красный акцент — сами брюки как тёмный низ не читались вовсе.
    "ЯКОРЬ ВИДЕН. Базовая вещь обязана быть УЗНАВАЕМОЙ в обоих образах: тот же крой и цвет, и она "
    "должна читаться на фото, а не растворяться тёмным пятном. Если якорь невыразителен, возьми "
    "другую вещь капсулы: приём «одна вещь» работает, только когда за ней следит глаз.\n"
    "НАЗВАНИЕ = СОДЕРЖАНИЕ. Не обещай «авангард» там, где классический костюм с акцентом: ярлык, "
    "который больше картинки, стилист и клиентка считывают сразу. Называй то, что собрано.\n"
    "В описании ПРЯМО назови якорь и что меняется: «общее — эти брюки; меняется верх и градус "
    "нарядности». Это педагогический смысл блока, без него остаются просто два образа.\n"
    "Сезон и обувь держи согласованными между образами: босоножки к шерстяному костюму — "
    "видимый конфликт.\n"
    "Описания — на русском, на «ты», через психологию запроса, без восклицательных знаков и штампов.\n"
    # Главный прокол пары на проде: на двух кадрах были РАЗНЫЕ женщины, и вся идея «гардероб
    # одного человека» рассыпалась. Причина — промпт не запрещал описывать внешность, модель
    # дописывала «woman with blonde hair», и рендер подхватывал чужое лицо вместо фото клиентки.
    "image_generation_prompt — БЕЗ ОПИСАНИЯ ЛИЦА, ВОЛОС, ВОЗРАСТА И ТЕЛОСЛОЖЕНИЯ: внешность "
    "берётся с фото клиентки, и любое её описание здесь ломает узнавание. В обоих образах пары "
    "человек ОДИН — пиши только одежду и сцену.\n"
