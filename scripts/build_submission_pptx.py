...
def build(prs: Presentation) -> None:
    ...
    _text(s, 0.9, 4.6, 11.5, 1.4,
          ...
          17, SANS, INK)
    _text(s, 0.9, 5.9, 11.5, 0.6, "Диагностика с двумя образами — 45–66 с; полная Карта с шестью образами — около 4 минут (живые прогоны на проде).",
    ...
    cards = [
        ...
        ("Тесты как контур", "417 автотестов держат агента: регресс закрывается тестом, а не правкой на глаз"),
    ...
    _notes(s, "2:30–3:10\n\n«Проект целиком построен в агентной разработке: 254 из 263 коммитов — в "
              ...
              "регламент, 417 тестов как страховка, агент в работе с данными и в исследовании. "
    ...
    _notes(s, "3:55–4:40\n\n«MVP работает end-to-end: полная Карта за минуту. Конверсия квиз→Карта 83%. "
              ...
              "Инженерия закрыта — репозиторий, Docker, CI, 417 тестов.»\n\n"
