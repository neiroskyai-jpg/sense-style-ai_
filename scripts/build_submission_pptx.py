...
def build(prs: Presentation) -> None:
    ...
    _text(s, 0.9, 2.15, 5.6, 1.6, "69.8%", 96, SERIF, WINE)
    _text(s, 0.9, 3.9, 5.6, 0.9, "средний стартовый разрыв\nна 8 замеренных клиентках", 14, SANS, MUTED)
    ...
    _notes(s, "4:00–4:40\n\n«MVP работает end-to-end: полная Карта за минуту. Инженерия закрыта — публичный "
              ...
              "повторных замера сделаны в один день (шум). Честная формулировка: «механика замера вшита "
              "в продукт, инфраструктура готова, лонгитюд требует недель — это следующий шаг». Эта "
              "честность работает так же, как оговорка про n=3.\n\nВИЗУАЛ: повторить кадр «до/после» с титула.")
