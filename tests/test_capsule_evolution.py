...
def test_constructor_keeps_own_items_without_photo_but_not_catalog_filler():
    """Своя вещь остаётся в конструкторе даже без фото; каталожный добор без фото — нет.

    Раньше правило было общим: нет картинки — вон из борда. Из-за него вылетала вся капсула
    клиентки: её вещи описаны языком метода («брюки прямого кроя, холодный чёрный») и по тексту
    в брендовом фиде почти не находятся. Конструктор заполнялся чужим каталогом целиком.

    Возражение, ради которого правило вводили — «пустые бежевые прямоугольники читаются как
    поломка», — снято не выбрасыванием вещи, а плиткой: слот и название на карточке
    (см. test_capsule_item_without_a_photo_gets_a_designed_tile).
    """
    own = [{"slot": "Верх", "items": [{"name": "Блузка", "image": "data:x"},
                                      {"name": "Топ без фото"}]}]
    extra = [{"slot": "Низ", "items": [{"name": "Брюки", "image": "data:y"},
                                       {"name": "Каталожный добор без фото"}]}]

    board = m._merge_boards(own, extra, limit=10)
    names = [it["name"] for grp in board for it in grp["items"]]

    assert "Топ без фото" in names, "вещь клиентки — предмет конструктора, а не украшение"
    assert "Каталожный добор без фото" not in names, "филлер без картинки бесполезен"
    assert set(names) == {"Блузка", "Топ без фото", "Брюки"}
