...
<div class=row2>
 ...
 <div class=panel id=wardrobe>
  ...
  <p class=psub>Собери образ из своей капсулы: нажми или перетащи вещь в ячейку справа.
   {% if season_built %}Это вещи из твоих образов — цвет каждой соответствует её плашке.
   {% else %}Своя капсула на этот сезон появится, когда соберёшь Карту. Ниже — вещи из уже собранной.{% endif %}</p>
  {% if season_tabs %}
  <div class=seasons>
   {% for s in season_tabs %}<a href="/cabinet?season={{ s.code }}" class="{{ 'on' if s.on else '' }}{{ ' notbuilt' if not s.built else '' }}" title="{{ 'капсула собрана' if s.built else 'соберётся, когда сделаешь Карту на этот сезон' }}">{{ s.label }}</a>{% endfor %}
  </div>
  {% endif %}
...
def cabinet():
    ...
    # Конструктор собирается ТОЛЬКО из вещей капсулы клиентки (capsule_items из её образов).
    # Раньше сюда подмешивался брендовый каталог — «добрать до размера» и дать фото к названиям.
    # Со свотчами фото не нужны (цвет берём из имени вещи), а чужой товар вперемешку клиентку
    # путал: «это моя капсула или магазин?». Источник теперь один — её гардероб.
    ...
    # capsule_items уже в порядке приоритета (из сборки Карты), поэтому «6 опорных» — это первые 6.
    board = _capsule_board(own[:items_n]) or card.get("capsule_board") or \
